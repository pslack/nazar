...
class eyeTracker{
    ...
    void update(cv::Mat & grayImgFromCam, float threshold, float minSize, float maxSize,  float minSquareness = 0.5);
		void draw(float x, float y, float w, float h);
	
    cv::Point getEyePoint();
		
		void setDownTime(int downWaitTimeMillis);
		void setUpTime(int upWaitTimeMillis);
	
    cv::Mat		grayImgPreModification;
		
		/*float scalef;
		float xoffset;
		float yoffset;*/
	
    cv::Mat		grayImgPreWarp;
	
    cv::Mat		grayImg;
    cv::Mat				colorImg;
    cv::Mat		threshImg;
    cv::Mat		contourFinder;
	
		void						calculateEdgePixels();
    cv::Point						edgeMaskStartPos;
    ...
    cv::Mat		edgeMask;
    cv::Mat			edgeMaskInverted;				// for display, easier to see what's going on.
    ...
    cv::Point currentEyePoint;
    cv::Point currentNormPoint;
